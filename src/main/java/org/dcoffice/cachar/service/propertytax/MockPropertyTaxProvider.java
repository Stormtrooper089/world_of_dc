...
@Service
public class MockPropertyTaxProvider implements PropertyTaxProvider {
    ...
    public MockPropertyTaxProvider(
    ...
    public Map<String, Object> getCitizenAccount(String citizenId) {
        ...
        response.put("services", List.of(
                ...
                serviceTile("Mutation / Transfer", "Submit owner or holding correction request", "ACTIVE"),
                serviceTile("Bifurcation / Amalgamation", "Submit split or merge request for review", "ACTIVE"),
                serviceTile("UPYOG Integration", "Provider-ready backend adapter", "READY")
        ));
        return response;
    }
    ...
    public List<PropertyTaxAccount> defaulters() {
        ...
        return propertyRepository.findAll().stream()
                .filter(property -> safe(property.getAmountDue()).compareTo(BigDecimal.ZERO) > 0)
                .sorted(Comparator.comparing(PropertyTaxAccount::getAmountDue, Comparator.nullsLast(Comparator.reverseOrder())))
                .collect(Collectors.toList());
    }
    ...
    private void ensureSeedProperties() {
